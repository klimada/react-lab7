...
class EmployeeNew extends React.Component {
    ...
    validation()
    {
      if(this.state.age >= 18)
      {
        ...
        emailTest.test(this.state.email) ? this.setState({emailisValid: true}) : this.setState({emailisValid: false});
       }
      else
      {
        const phoneTest = /[0-9]{9}/;
        phoneTest.test(this.state.phone) ? this.setState({phoneisValid: true}) : this.setState({phoneisValid: false});
       }
    }

    render() { 
        const { age,isActive,email,phone,emailisValid,phoneisValid} = this.state;
        return (
            <div>
            <form style={{ margin: "15px 0 0 50px" }}  onSubmit={event => {event.preventDefault(); this.validation()}}> 
                ...
                <br></br>
                {(age >= 18) ? <div><label style={{ margin: "15px 0 0 50px" }}> Email: <input type="text" value={email} name="email" onChange={this.somethingChanged} /></label>
                              {!emailisValid ? <label style={{fontSize:"10px",color:"red"}}>Incorrect email address</label> : null}</div>
                             : <label style={{ margin: "15px 0 0 50px" }}> Parent Phone No: <input type="text" value={phone} name="phone" onChange={this.somethingChanged}/></label>}
